...
(defun best-search (problem)
  (let*  ((state (problem-initial-state problem))
      (h (euclidean-distance state))
      (node (make-node :parent nil :state state :g 0 :h h :f h))
      (Q '())
      (visited '()))
  (setf Q (cons node Q))
  (loop while (not (null Q)) do
    (setf node (pop Q))
    (setf visited (cons (node-state node) visited))
    (setf state (node-state node))
    (loop for next_state in (bfs-nextStates problem state visited) do
      (let* ((g (+ (state-cost next_state) (node-g node)))
          (h (euclidean-distance state))
          (new_node (make-node :parent node :state next_state :f (+ g h) :g g :h h)))
      (if (funcall (problem-fn-isGoal problem) next_state)
        (return-from best-search (solution new_node)))
      (setf Q (cons new_node Q))
      (setf Q (oderer Q))
    ))
  )
))
(defun bfs-nextStates (problem state visited)
  (let ((lst '()))
  (loop for next_state in (funcall (problem-fn-nextStates problem) state) do
    (if (not (state-visited next_state visited))
      (setf lst (cons next_state lst))
    )
  )
  (return-from bfs-nextStates lst)
))
(defun state-visited (state visited)
  (loop for st in visited do
    (if (equal st state) 
      (return-from state-visited t))
  )
  nil
)
(defun euclidean-distance (st)
  (let* ((state (state-track st))
       (goals (track-endpositions (state-track st)))
       (pos (state-pos st))
       (distance most-positive-fixnum))
  (if (isObstaclep pos state) 
    (return-from euclidean-distance distance))
  (loop for goal in goals do
    (setf distance (min distance (cal-distance pos goal)))
  )
  (return-from euclidean-distance (round distance))
))
(defun cal-distance (initial goal)
  (sqrt (+ (expt (- (nth 0 initial) (nth 0 goal)) 2) 
       (expt (- (nth 1 initial) (nth 1 goal)) 2)))
)